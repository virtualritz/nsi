...
| A new `EdgeKind` cannot land silently | Covered | `scene/mod.rs` `affected` matches on `EdgeKind` with no wildcard | Compile-time: removing an arm fails the build. It matched `to_attribute()` strings before, where a new variant fell into the `_` arm and a `ShaderNetwork` port named `objects` took the child arm | -- |
...
| Over-approximation is bounded | Covered | `scene/mod.rs` `affected` inserts roots without an ancestor check; `scene/mod.rs` `collapsed_roots` removes the covered ones on request | `scene::tests::a_root_under_another_root_is_collapsed_away`, `unrelated_roots_all_survive_collapsing`, `a_multi_parent_root_is_kept`; removing the multi-parent guard reddens the third. **Measured before deciding**, on a 1791-node asset hierarchy, release build: a clustered batch (a transform and 20 of its descendants) yields 21 roots of which **20 are redundant**, and walking all of them visits 1785 nodes in 200 us against 597 in 56 us collapsed -- 144 us saved for a 3.6 us check. A scattered batch (21 unrelated leaves) yields **no** overlap and pays 11.3 us for nothing, against a 2.4 us walk. So overlap is common enough to be worth removing and cheap enough to leave to the caller: `affected` stays as it was, and a consumer that walks subtrees opts in | -- |
