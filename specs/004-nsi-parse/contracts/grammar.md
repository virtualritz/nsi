...
| A count that does not match the values is **refused**, where 3Delight warns | Open | `parse.rs` requires as many values as the declared count | None -- the refusal is tested, the divergence is not | Rendered: `"transformationmatrices" "doublematrix" 1` with **thirty-two** doubles logs `E1000 P1104: array of 'double's has wrong size, expected 16 ... but given 32` as a *warning* and 3Delight **renders**, drawing one instance from the first sixteen. This parser rejects the stream, so a `.nsi` file the renderer reads is one this crate cannot. Refusing is the safer default -- silently keeping a prefix is how a wrong picture gets built -- but the choice should be the caller's, and it is not recorded anywhere the caller can see. Found by a reviewer. |
