...
| The expansion rule is recorded for consumers | Covered | `lib.rs` crate docs; `research.md` Q1 | Measured, and wider than the row's name suggests: **any** `${VAR}` expands, not only `NSI_PATH_`-prefixed ones -- `${HOME}` and `${OTHER_VAR}` both resolved. The `NSI_PATH_` prefix governs which variables 3Delight *writes* as references under `streampathreplacement`, a write-side convention, not which it reads. A backend that uses a path value without expanding opens the wrong file | -- |
