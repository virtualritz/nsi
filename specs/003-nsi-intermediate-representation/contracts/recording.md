...
| `Node::effective` is the rule, not `attrs` | Covered | `scene/mod.rs` `Node::effective`, read by the resolver | Same tests. A public `Node::get` reading `attrs` alone would have reinstated the silent wrong answer fixed two commits earlier -- an attribute set only with `SetAttributeAtTime` is honoured by 3Delight and invisible in `attrs`. One method, used by the resolver and offered to callers, so the two cannot disagree  The **order** is part of the rule: the value is the last call's, not the greatest time's, which `Node::sample_order` records and `scene::tests::effective_takes_the_last_call_not_the_greatest_time` pins against two renders that answer oppositely. | -- |
| A hand-built node cannot panic a reader | Covered | `resolve/mod.rs` `sampled_attr` returns `Unset` for an empty call log rather than unwrapping a survivor | `scene::tests::a_hand_built_node_does_not_panic_the_readers`; unwrapping again reddens it with the panic itself. `Node`'s fields are public, so a caller can build one no recorder would -- and a resolver that aborted a render over it would be a worse answer than an attribute that goes quiet. The reachable states narrowed sharply when the three tables that had to agree became one call log | -- |
