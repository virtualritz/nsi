...
| A hand-edited node cannot panic a reader | Covered | `resolve/mod.rs` `sampled_attr` and both emitters skip a time `Node::sample_order` names and `Node::time_attrs` does not hold | `scene::tests::a_hand_edited_node_does_not_panic_the_readers`; unwrapping the lookup again reddens it with a panic. Every field of a `Node` is public, so the pair the recorder keeps in step is one a caller can put out of step -- and a resolver that aborted a render over it would be a worse answer than an attribute that goes quiet | -- |
