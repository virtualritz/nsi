...
| `Recorder` is `Send + Sync` | Covered | `owned/mod.rs` `HostPointer` + `unsafe impl Send/Sync` | `recorder::tests::recorder_is_send_and_sync`. Proves the auto-trait only; the soundness argument is the `'static` pin on the `Arg` GAT, which is an argument, not a test. | -- |
...
| Scalar payloads are copied during the call | Covered | `owned/mod.rs` `OwnedArgument::from_param` | `owned::tests::owns_a_single_f32`, `owns_a_string` | -- |
...
| `disconnect` honours `.all` in all four positions | Covered | `scene/mod.rs` `disconnect`, `EdgeKind::to_attribute`, `lib.rs` `ALL` | `scene::tests::disconnect_all_matches_every_source` (ɴsɪ's own documented example), `disconnect_all_matches_destinations_and_attributes`, `disconnect_all_matches_every_source_attribute` (the source-attribute position, which was a silent no-op), `disconnect_with_an_all_attribute_is_not_a_classify_error` | -- |
...
| A repeated `connect` updates rather than duplicates | Covered | `scene/mod.rs` `connect_with_arguments` matches on `(from, to, kind)` | `scene::tests::a_repeated_connect_updates_rather_than_duplicates`; without it the node reads as having two parents and its whole subtree fails to resolve | -- |
...
- A `HostPointer` is never dereferenced by this crate.
...
- To close the `.all` row: a test disconnecting with `nsi::ALL` as the
  `to` handle and as the `to_attribute`, asserting the documented outcome.
...
| `Node::effective` is the rule, not `attributes` | Covered | `scene/mod.rs` `Node::effective`, read by the resolver | Same tests. A public `Node::get` reading `attributes` alone would have reinstated the silent wrong answer fixed two commits earlier -- an attribute set only with `SetAttributeAtTime` is honoured by 3Delight and invisible in `attributes`. One method, used by the resolver and offered to callers, so the two cannot disagree  The **order** is part of the rule: the value is the last call's, not the greatest time's, which the call log records and `scene::tests::effective_takes_the_last_call_not_the_greatest_time` pins against two renders that answer oppositely. | -- |
