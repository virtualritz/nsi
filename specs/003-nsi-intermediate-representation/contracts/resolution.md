...
| Per-path motion and per-path attributes | Open | `placements` refuses a sampled node; `attribute_value` and `shader_attributes` take a geometry, not a path | None | A *moving instanced* geometry has no answer anywhere: `placements` gives `MotionSampledTransform` and `world_transform_interpolated_at` gives `MultipleParents`. Likewise `attribute_value` and `shader_attributes` refuse a multi-parent node, so the `ATTR.priority` and `visibility.<ray>` rules cannot be applied along a placement -- `Placement::binding` lists the nodes without resolving them. The path-based internals exist (`gathered_along`, `compose_along`); this is exposing them as `placements_interpolated_at(geometry, time)` and an `attribute_value_along(path, name)`. |
