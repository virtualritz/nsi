...
| Attributes provided through a `set` | Open | `resolve/mod.rs` `gathered_containers` walks the geometry-to-root chain only; `SetMember` edges are never consulted | None | ɴsɪ connects `shaderattributes` to "geometric primitives, transform nodes **or set nodes**", and 3Delight honours **`geometryattributes` on a set too** (`T12`: an `attributes` node with `visibility 0` on `s.geometryattributes`, the mesh a `members` of `s`, renders invisible) even though §3.12.1 lists only primitives and transforms. So *both* containers have this blind spot, and both are silent -- `geometry_binding` and `attribute_value` share it, which was untracked until round 12. The ranking is no longer unknown: rendered, it is **the primitive's own attributes, then its `shaderattributes`, then the set's, then the transform's** (`T13`, `T14`, `T15`). Closing it means walking `SetMember` as a second source at that rank, and deciding how multiple or nested set memberships order. |
