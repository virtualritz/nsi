...
| A `shaderattributes` node on a `set` | Open | `resolve/mod.rs` walks the geometry-to-root chain only | None | ɴsɪ connects these to "geometric primitives, transform nodes **or set nodes**". A set is not on the transform chain, so a shader attribute provided through set membership is not found. Closing it means walking `SetMember` edges as a second source and deciding how a set ranks against the chain, which the specification does not say. |
