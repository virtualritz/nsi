...
| A moving instancer is answered, not reported empty | Covered | `resolve/mod.rs` `instance_transforms_at`, and `instance_transforms` now refuses sampled matrices | `resolve::tests::a_moving_instancer_is_refused_not_reported_empty`, `instance_transforms_at_agrees_on_a_static_instancer`. Reporting empty instead of refusing, ignoring the time, and not holding the end sample each redden the first. Rendered: an instancer with sampled matrices draws two blurred copies where the crate reported none | -- |
