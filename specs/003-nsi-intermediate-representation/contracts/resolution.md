...
| Highest priority wins, then proximity | Covered | `resolve/mod.rs` `sort_by` | `resolve::tests::priority_beats_proximity`, `the_nearest_binding_wins_at_equal_priority` | -- |
...
| `ATTR.priority` and visibility specificity | Open | Not implemented | None | ɴsɪ has a *second* priority: "`ATTR.priority` ... sets the priority of attribute ATTR when gathering attributes in the scene hierarchy", set on the `attributes` node itself and distinct from the connection `priority` this resolver sorts by. At equal priority a more specific `visibility.<ray>` also beats `visibility`. `Binding::attributes` tells a backend to take the first node that defines an attribute, which is wrong for any scene that sets either. Surface it as `Scene::attribute_value(geometry, name)` or document both rules on `Binding`. |
