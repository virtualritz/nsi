...
| Attributes provided through a `set` | Covered | `resolve/mod.rs` `gathered_containers` walks `SetMember` edges from the geometry as a source between it and its first transform | Seven tests in `resolve::tests` (`an_attributes_node_on_a_set_binds`, `the_geometrys_own_container_outranks_its_set`, `a_set_outranks_the_transform_above_it`, `the_first_set_membership_wins`, `a_nested_sets_attributes_are_not_inherited`, `an_attr_priority_on_a_set_beats_the_geometrys_own`, `a_set_provides_shader_attributes_below_the_geometry`). Dropping sets, ranking them above the geometry, ranking them below the transforms, and walking them transitively each redden a different one. Rendered in 3Delight with both directions mirrored, so no result is an artefact of which value was `0`: a container on the geometry beats one on its set (`G1`/`G1m`); the set beats the transform above it (`G2`/`G2m`); `ATTR.priority` on a set still outranks proximity (`G3`); with two memberships the first connection wins (`G4`/`G4m`); a set nested in another set contributes nothing (`G5`). Controls: an empty scene renders visible, and a lone set-attached `visibility 0` renders invisible | -- |
