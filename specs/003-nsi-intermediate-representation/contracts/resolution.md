...
| A non-integer `ATTR.priority` is ignored | Covered | `resolve/mod.rs` `priority_value` reads `I32`/`I64` only | `resolve::tests::a_non_integer_priority_is_ignored`; reading an `F32` as one reddens it | -- |
