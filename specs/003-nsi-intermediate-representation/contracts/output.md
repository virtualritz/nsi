...
| zstd round-trips | Covered | `stream/mod.rs` `write_stream_with`, `zstd::stream::write::Encoder` | `compression::zstd_decompresses_to_the_plain_stream` | -- |
...
| A compressor is finished, not merely dropped | Covered | `stream/mod.rs` calls `finish()` on both encoders | The round-trip tests. Note `GzEncoder` also finishes on `Drop`, so the explicit call buys error propagation rather than correctness; `zstd`'s does not, and dropping it fails the round-trip | -- |
| The extension names the compressor | Covered | `stream/mod.rs` `Compression::extension` | `compression::the_extension_names_the_compressor` | -- |
...
| `binarynsi` is written | Open | Not implemented | None | The encoding is undocumented; implementing it means reading 3Delight's output byte by byte, as the text format was. A backend can always write text, which 3Delight reads, so this is an optimisation rather than a gap. |
| `binarynsi` is *recognised* on read | Covered | `nsi-parse` `parse_stream` sniffs the header | `roundtrip::a_binary_stream_is_named_as_such`; removing the sniff reddens it. `renderdl -cat -binary` output begins `cc 00` whatever statement comes first -- the third byte is a length tag for the keyword -- and `autonsi`, 3Delight's default for a name not ending `.nsia`, selects binary, so a caller can be handed one they did not choose. It used to fail as "at byte 0: not UTF-8", which is true and sends the reader after an encoding problem instead of a format one. The second header byte is not separately proven: narrowing to `0xCC` alone leaves the suite green, and nothing can distinguish them, because a text stream must start with a keyword | -- |
| A compressed Lua script | Covered | Not offered, deliberately | 3Delight does **not** read one. `renderdl -lua -cat` on a gzipped script fails with `E6012 LUA error : ...: unexpected symbol near '<\31>'` -- it hands the gzip magic byte straight to the Lua lexer -- under a `.lua` name and a `.lua.gz` name alike, while the same script uncompressed parses. Offering compressed Lua would produce a file nothing consumes | -- |
