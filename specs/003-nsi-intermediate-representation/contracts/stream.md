...
| Motion samples emit in the order they were **set** | Covered | `stream/mod.rs` `write_stream` walks `Node::sample_order`, not `time_attrs` | `nsi-parse` `cross_crate::the_order_the_samples_were_set_in_survives_the_round_trip`; walking `time_attrs` reddens it. An attribute resolves by the last *call*, so a writer that emitted the timeline handed the reader a scene that resolves differently from the one it wrote -- silently, and in the direction that matters: `t=1` set before `t=0` renders the `t=0` value. `write_lua` does the same | -- |
